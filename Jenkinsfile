...
def login(String server, String creds){
    ...
    withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: "${apikey}",
    ...
    stage('init') {
      steps {
        script{
          ...
          try {
            echo "Attempting Login to ${envs['devServer']}"
            login(${envs['devServer']}, creds)
          } catch(exe){
            echo "Failed to Login to ${envs['devServer']} with Status Code: ${exe}"
            throw exe
          }
      }                         
  }                  
